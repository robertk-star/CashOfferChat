(() => {
  ...
  function getSettingsValue(...keys) {
    for (const key of keys) {
      if (state.settings && state.settings[key] !== undefined && state.settings[key] !== null && state.settings[key] !== "") {
  ...
  function validateForm() {
    ...
    if (!state.form.propertyCity.trim() && !state.form.propertyAddress.trim()) {
      return "Property city or property address is required.";
    }
    return "";
  }
  ...
  async function sendMessage(text) {
    ...
    try {
      ...
      const reply =
        ...
        data.message ||
        data.answer ||
        "I can help with questions about selling a house as-is. If you want the team to review the property, use the quote button and enter the house details.";
      ...
      state.messages.push({
        role: "assistant",
        content:
          "I had trouble answering that. You can still enter house information for a quote review using the button above.",
      });
    } finally {
      state.loading = false;
      render();
  ...
  function renderHeader() {
    ...
    const actions = [
      el(
        "button",
        { class: "coc-primary-button", type: "button", onclick: () => setView("form") },
        [state.settings.widgetQuoteButtonText || DEFAULT_SETTINGS.widgetQuoteButtonText]
      ),
    ];
    ...
    if (showCall) {
      actions.push(
        el(
          "a",
          { class: "coc-secondary-button", href: `tel:${phone.replace(/[^\d+]/g, "")}` },
  ...
  function renderChatBody() {
    ...
    state.messages.forEach((message) => {
      chat.appendChild(
        el("div", { class: `coc-message ${message.role === "user" ? "coc-user" : "coc-assistant"}` }, [
          message.content,
        ])
      );
    });
    ...
    if (state.loading) {
      chat.appendChild(el("div", { class: "coc-message coc-assistant" }, ["Thinking..."]));
    }
    ...
    chat.appendChild(
      el(
        ...
        quickReplies.map((reply) =>
          el("button", { class: "coc-chip", type: "button", onclick: () => sendMessage(reply) }, [reply])
        )
      )
    );
  ...
  function renderFormBody() {
    const form = el("div", { class: "coc-form" }, [
      ...
      el("div", { class: "coc-help" }, [
        "Required: name, phone, and either property city or property address. There is no obligation to accept an offer.",
      ]),
      el("div", { class: "coc-grid" }, [
        field("Property city", "propertyCity", "Plano"),
        field("Property address", "propertyAddress", "123 Main St"),
        selectField("What best describes the situation?", "situation", [
          "",
          "Just exploring",
          "Need to sell fast",
          "Needs repairs",
        ...
        el(
          "button",
          { class: "coc-primary-button", type: "button", disabled: state.loading, onclick: submitLead },
          [state.loading ? "Submitting..." : "Submit My House Info"]
        ),
      ]),
    ]);
  ...
  function renderPanel(root) {
    ...
    panel.appendChild(el("div", { class: "coc-footer-note" }, [
      "CashOfferChat helps collect seller information for follow-up. It does not make offers or provide legal, tax, or financial advice.",
    ]));
