...
async function replaceNamedRows(supabase: any, table: string, businessId: string, names: string[]) {
  await supabase.from(table).delete().eq("business_id", businessId);
  if (names.length) await supabase.from(table).insert(names.map((name) => ({ business_id: businessId, name })));
}
...
export async function POST(request: Request) {
  ...
  if (!supabase) return NextResponse.redirect(new URL("/admin/settings?error=1", request.url), { status: 303 });

  const formData = await request.formData();
  const businessId = value(formData, "business_id");
  if (!businessId) return NextResponse.redirect(new URL("/admin/settings?error=1", request.url), { status: 303 });
  ...
  if (settingsError) return NextResponse.redirect(new URL("/admin/settings?error=1", request.url), { status: 303 });

  await replaceNamedRows(supabase, "service_areas", businessId, parseLines(value(formData, "service_areas")));
  await replaceNamedRows(supabase, "referral_areas", businessId, parseLines(value(formData, "referral_areas")));

  await supabase.from("property_buying_criteria").delete().eq("business_id", businessId);
  ...
  const faqs = faqQuestions.map((question, index) => ({
    question,
  ...
  if (newQuestion && newAnswer) faqs.push({ question: newQuestion, answer: newAnswer, removed: false } as any);

  if (faqs.length) {
    await supabase.from("managed_faq_items").insert(faqs.map((faq, index) => ({
      business_id: businessId,
