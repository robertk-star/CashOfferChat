...
export default async function ClientSiteDetailPage({
  ...
  const { data: site } = await supabase
    .from("widget_sites")
    .select("id, business_id, site_id, name, site_name, domain, allowed_domains, is_active, created_at, updated_at")
    .eq("id", id)
    ...
    <main className="min-h-screen bg-slate-50">
      <header className="border-b border-slate-200 bg-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-5">
          <div>
            <Link href="/client/sites" className="text-sm font-bold text-slate-500 underline">Back to Widget Sites</Link>
            <h1 className="mt-2 text-2xl font-bold text-navy">{label}</h1>
            <p className="text-sm text-slate-500">{businessName} · Site ID: {site.site_id}</p>
          </div>
      ...
      <section className="mx-auto grid max-w-7xl gap-6 px-6 py-8 lg:grid-cols-[.9fr_1.1fr]">
        <div className="space-y-6">
          ...
          {query.error && <div className="rounded-2xl bg-red-50 p-4 text-sm text-red-700">Widget site could not be saved.</div>}

          <div className="rounded-[2rem] bg-white p-6 shadow-soft ring-1 ring-slate-200">
            <h2 className="text-xl font-bold text-navy">Site Settings</h2>
            <p className="mt-2 text-sm text-slate-500">You can update display details and allowed domains for this widget site.</p>

            <form action={`/api/client/sites/${site.id}`} method="post" className="mt-6 space-y-4">
              ...
              <label className="block text-sm font-semibold text-slate-700">
                Primary Domain
                <input name="domain" defaultValue={site.domain || ""} placeholder="example.com" className="mt-1 w-full rounded-xl border border-slate-300 px-4 py-3" />
              </label>

              <label className="block text-sm font-semibold text-slate-700">
                Allowed Domains
                <textarea name="allowed_domains" defaultValue={site.allowed_domains || ""} placeholder={"example.com\nwww.example.com"} className="mt-1 min-h-32 w-full rounded-xl border border-slate-300 px-4 py-3" />
              </label>
