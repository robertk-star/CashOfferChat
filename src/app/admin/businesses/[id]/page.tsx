...
export default async function AdminBusinessDetailPage({
  ...
  searchParams: Promise<{ saved?: string; error?: string; leadDeleted?: string }>;
}) {
  ...
  const [sitesResult, usersResult, leadsResult] = await Promise.all([
    supabase.from("widget_sites").select("id, site_id, name, site_name, domain, allowed_domains, is_active").eq("business_id", id).order("created_at", { ascending: false }),
    supabase.from("business_users").select("id, email, name, role, is_active, last_login_at").eq("business_id", id).order("created_at", { ascending: false }),
    supabase.from("seller_leads").select("id, created_at, name, phone, property_address, property_city, status").eq("business_id", id).order("created_at", { ascending: false }).limit(25),
  ]);
    ...
    <main className="min-h-screen bg-slate-50">
      <header className="border-b border-slate-200 bg-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-5">
          <div>
            <Link href="/admin/businesses" className="text-sm font-bold text-slate-500 underline">Back to Businesses</Link>
            <h1 className="mt-2 text-2xl font-bold text-navy">{business.name}</h1>
            <p className="text-sm text-slate-500">{business.slug || business.id}</p>
          </div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-8">
        {query.saved && <div className="mb-6 rounded-2xl bg-green-50 p-4 text-sm text-green-800">Business saved.</div>}
        {query.error && <div className="mb-6 rounded-2xl bg-red-50 p-4 text-sm text-red-700">Business could not be saved.</div>}
        ...
        <div className="grid gap-6 lg:grid-cols-[.9fr_1.1fr]">
          <div className="rounded-[2rem] bg-white p-6 shadow-soft ring-1 ring-slate-200">
            ...
            <form action={`/api/admin/businesses/${id}`} method="post" className="mt-6 space-y-4">
              <label className="block text-sm font-semibold text-slate-700">Business Name *<input name="name" defaultValue={business.name || ""} required className="mt-1 w-full rounded-xl border border-slate-300 px-4 py-3" /></label>
              <label className="block text-sm font-semibold text-slate-700">Website<input name="website" defaultValue={business.website || ""} className="mt-1 w-full rounded-xl border border-slate-300 px-4 py-3" /></label>
              <label className="block text-sm font-semibold text-slate-700">Phone<input name="phone" defaultValue={business.phone || ""} className="mt-1 w-full rounded-xl border border-slate-300 px-4 py-3" /></label>
          ...
          <div className="space-y-6">
                ...
                {sites.map((site) => {
                  ...
                  return (
                    <div key={site.id} className="rounded-2xl border border-slate-200 p-4">
                      ...
                      <pre className="mt-3 overflow-x-auto rounded-xl bg-slate-50 p-3 text-xs text-slate-700">{buildWidgetEmbedCode(site.site_id)}</pre>
                    </div>
                  );
                })}
              </div>
            ...
            <div className="rounded-[2rem] bg-white p-6 shadow-soft ring-1 ring-slate-200">
              <div className="flex items-center justify-between"><h2 className="text-xl font-bold text-navy">Client Users</h2><Link href="/admin/clients" className="text-sm font-bold text-navy underline">Manage Users</Link></div>
              <div className="mt-5 space-y-3">
                {users.length === 0 && <p className="text-sm text-slate-500">No client users yet.</p>}
                {users.map((user) => <div key={user.id} className="flex items-center justify-between rounded-2xl bg-slate-50 p-4"><div><div className="font-bold text-navy">{user.name || user.email}</div><div className="text-sm text-slate-500">{user.email}</div></div><Link href={`/admin/clients/${user.id}`} className="text-sm font-bold text-navy underline">Open</Link></div>)}
              </div>
            </div>
            ...
            <div className="rounded-[2rem] bg-white p-6 shadow-soft ring-1 ring-slate-200">
              ...
              <div className="mt-5 space-y-3">
                ...
                {leads.map((lead) => <div key={lead.id} className="flex items-center justify-between rounded-2xl bg-slate-50 p-4"><div><div className="font-bold text-navy">{lead.name || "Seller Lead"}</div><div className="text-sm text-slate-500">{lead.phone || "No phone"} · {lead.property_city || lead.property_address || "No property location"}</div></div><Link href={`/admin/leads/${lead.id}`} className="text-sm font-bold text-navy underline">Open</Link></div>)}
              </div>
            </div>
          </div>
        </div>
