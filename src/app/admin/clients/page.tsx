      ...
      <section className="mx-auto max-w-7xl px-6 py-8">
        ...
        <div className="overflow-hidden rounded-[2rem] bg-white shadow-soft ring-1 ring-slate-200">
          <table className="min-w-full divide-y divide-slate-200 text-sm">
            <thead className="bg-slate-100 text-left text-xs uppercase tracking-wide text-slate-500"><tr><th className="px-5 py-4">Client</th><th className="px-5 py-4">Business</th><th className="px-5 py-4">Active</th><th className="px-5 py-4">Last Login</th></tr></thead>
            <tbody className="divide-y divide-slate-100">
              {users.length === 0 && <tr><td className="px-5 py-8 text-center text-slate-500" colSpan={4}>No client users yet.</td></tr>}
              {users.map((u) => <tr key={u.id}><td className="px-5 py-4"><div className="font-bold text-navy">{u.name || "—"}</div><div className="text-slate-500">{u.email}</div></td><td className="px-5 py-4 text-slate-600">{getBusinessName(u.businesses)}</td><td className="px-5 py-4 text-slate-600">{u.is_active ? "Yes" : "No"}</td><td className="px-5 py-4 text-slate-600">{u.last_login_at ? new Date(u.last_login_at).toLocaleString() : "—"}</td></tr>)}
            </tbody>
          </table>
        </div>
      </section>
