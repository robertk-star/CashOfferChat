...
export async function POST(request: Request) {
  ...
  if (!currentPassword || newPassword.length < 8 || newPassword !== confirmPassword) {
    return NextResponse.redirect(new URL("/client/account?error=1", request.url), { status: 303 });
  }
  ...
  if (!user || !user.is_active || !verifyClientPassword(currentPassword, user.password_hash)) {
    return NextResponse.redirect(new URL("/client/account?error=1", request.url), { status: 303 });
  }
  ...
  const { error } = await supabase
    .from("business_users")
    .update({
      password_hash: hashClientPassword(newPassword),
