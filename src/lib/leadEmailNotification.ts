...
function getAppUrl() {
  const rawUrl = process.env.APP_URL || process.env.NEXT_PUBLIC_APP_URL || "https://www.cashofferchat.com";

  try {
    const parsed = new URL(rawUrl);
    if (parsed.hostname === "cashofferchat.com" || parsed.hostname === "www.cashofferchat.com") {
...
async function getLeadNotificationRecipients(supabase: SupabaseClientLike, businessId?: string | null) {
  ...
  const clientRecipients = (data || []).flatMap((user: { email?: string | null }) => splitEmails(user.email));
  return uniqueEmails([...clientRecipients, ...envRecipients]);
}

function buildLeadEmail({ lead, businessName }: { lead: LeadRow; businessName?: string | null }) {
  ...
  const subject = `New CashOfferChat lead: ${textValue(lead.name)}${lead.property_city ? ` in ${lead.property_city}` : ""}`;
  ...
  const rows = [
    ...
    ["Property City", lead.property_city],
    ["Situation", lead.situation],
    ["Timeline", lead.timeline],
    ["Property Condition", lead.property_condition],
    ["Notes", lead.notes],
    ["Source URL", lead.source_url],
    ["Site ID", lead.site_id],
    ["Submitted", lead.created_at ? new Date(lead.created_at).toLocaleString("en-US", { timeZone: "America/Chicago" }) : null],
  ];
  ...
  const textRows = rows.map(([label, value]) => `${label}: ${textValue(value)}`).join("\n");
      ...
      <div style="font-family:Arial,sans-serif;background:#f8fafc;padding:24px;color:#0f172a;">
        <div style="max-width:680px;margin:0 auto;background:#ffffff;border:1px solid #e5e7eb;border-radius:18px;overflow:hidden;">
          <div style="background:#0f172a;color:#ffffff;padding:22px 24px;">
            <h1 style="margin:0;font-size:22px;line-height:1.3;">New CashOfferChat Lead</h1>
            <p style="margin:8px 0 0;color:#cbd5e1;">${escapeHtml(safeBusinessName)}</p>
          </div>
          <div style="padding:24px;">
            <p style="margin:0 0 18px;color:#334155;">A seller submitted their information through the widget.</p>
...
export async function sendLeadEmailNotification({
  ...
  const recipients = await getLeadNotificationRecipients(supabase, lead.business_id);

  if (!resendApiKey) {
    return { skipped: true, sent: false, recipients, error: "RESEND_API_KEY is not configured." };
  }

  if (recipients.length === 0) {
    return { skipped: true, sent: false, recipients, error: "No notification recipients found." };
  }
