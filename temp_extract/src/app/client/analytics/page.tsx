...
export default async function ClientAnalyticsPage() {
  ...
  if (!supabase) {
    ...
    const [businessResult, eventsResult, leadsResult, sitesResult] = await Promise.all([
      supabase.from("businesses").select("name").eq("id", session.businessId).maybeSingle(),
      supabase
        .from("widget_events")
        .select("id, created_at, event_type, site_id, domain, page_url, lead_id, conversation_id")
      ...
      supabase
        .from("seller_leads")
        .select("id, created_at, name, phone, email, property_city, property_address, status, site_id")
        .eq("business_id", session.businessId)
        .gte("created_at", since7)
        .order("created_at", { ascending: false })
        .limit(250),
  ...
  const openedEvents = events.filter((event) => event.event_type === "widget_opened").length;
  const formOpenedEvents = events.filter((event) => event.event_type === "quote_form_opened").length;
  const leadSavedEvents = events.filter((event) => event.event_type === "lead_saved" || event.event_type === "lead_submitted").length;
      ...
      <header className="border-b border-slate-200 bg-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-5">
          <div>
            ...
            <p className="text-sm text-slate-500">{businessName} · Last 7 days</p>
          </div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-8">
        ...
        <div className="mb-8 grid gap-4 md:grid-cols-2 lg:grid-cols-5">
          <div className="rounded-3xl bg-white p-6 shadow-soft ring-1 ring-slate-200">
            <p className="text-sm font-semibold text-slate-500">Widget Events</p>
          ...
          <div className="rounded-3xl bg-white p-6 shadow-soft ring-1 ring-slate-200">
            <p className="text-sm font-semibold text-slate-500">Quote Form Opens</p>
            <p className="mt-2 text-4xl font-bold text-navy">{formOpenedEvents}</p>
          </div>
          <div className="rounded-3xl bg-white p-6 shadow-soft ring-1 ring-slate-200">
            <p className="text-sm font-semibold text-slate-500">Lead Rate</p>
            <p className="mt-2 text-4xl font-bold text-navy">{percent(leads.length || leadSavedEvents, events.length)}%</p>
          </div>
        </div>
        ...
        <div className="mb-8 rounded-[2rem] bg-white p-6 shadow-soft ring-1 ring-slate-200">
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            ...
            {sites.map((site) => {
              ...
              const siteEventCount = events.filter((event) => event.site_id === site.site_id).length;
              const siteLeadCount = leads.filter((lead) => lead.site_id === site.site_id).length;
                ...
                <div key={site.id} className="rounded-2xl border border-slate-200 p-4">
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-2 text-sm">
                    <div className="rounded-xl bg-slate-50 p-3">
                      <div className="text-xs font-bold uppercase text-slate-500">Events</div>
                      <div className="text-xl font-bold text-navy">{siteEventCount}</div>
                    </div>
                    <div className="rounded-xl bg-slate-50 p-3">
                      <div className="text-xs font-bold uppercase text-slate-500">Leads</div>
                      <div className="text-xl font-bold text-navy">{siteLeadCount}</div>
                    </div>
                  </div>
                </div>
              );
        ...
        <div className="mb-8 grid gap-6 lg:grid-cols-3">
          <div className="rounded-[2rem] bg-white p-6 shadow-soft ring-1 ring-slate-200">
            ...
            <div className="mt-4 space-y-2">
              {eventTypes.length === 0 && <p className="text-sm text-slate-500">No events yet.</p>}
              {eventTypes.map(([label, count]) => (
                <div key={label} className="flex justify-between rounded-xl bg-slate-50 px-4 py-2 text-sm">
                  <span>{label}</span>
                  <strong>{count}</strong>
                </div>
              ))}
          ...
          <div className="rounded-[2rem] bg-white p-6 shadow-soft ring-1 ring-slate-200">
            ...
            <div className="mt-4 space-y-2">
              {domains.length === 0 && <p className="text-sm text-slate-500">No domains yet.</p>}
          ...
          <div className="rounded-[2rem] bg-white p-6 shadow-soft ring-1 ring-slate-200">
            ...
            <div className="mt-4 space-y-2">
              {siteIds.length === 0 && <p className="text-sm text-slate-500">No site activity yet.</p>}
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <div className="rounded-[2rem] bg-white p-6 shadow-soft ring-1 ring-slate-200">
            ...
            <div className="mt-5 space-y-3">
              {leads.length === 0 && <p className="text-sm text-slate-500">No leads in the last 7 days.</p>}
              {leads.slice(0, 25).map((lead) => (
                <div key={lead.id} className="flex items-center justify-between rounded-2xl bg-slate-50 p-4">
                  ...
                  <Link href={`/client/leads/${lead.id}`} className="text-sm font-bold text-navy underline">Open</Link>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-[2rem] bg-white p-6 shadow-soft ring-1 ring-slate-200">
            ...
            <div className="mt-5 space-y-3">
              {events.length === 0 && <p className="text-sm text-slate-500">No events in the last 7 days.</p>}
              {events.slice(0, 25).map((event) => (
                <div key={event.id} className="rounded-2xl bg-slate-50 p-4">
                  <div className="flex items-center justify-between gap-4">
                    <div className="font-bold text-navy">{event.event_type}</div>
                    <div className="text-xs text-slate-500">{formatDate(event.created_at)}</div>
                  </div>
                  <div className="mt-1 text-sm text-slate-500">{event.domain || "No domain"} · {event.site_id || "No site"}</div>
                  {event.page_url && <div className="mt-1 break-all text-xs text-slate-400">{event.page_url}</div>}
                </div>
              ))}
            </div>
          </div>
